...
- 자세 참조 입력: PNG·JPEG 1개를 선택해 원본, `가로×세로`, 전체 픽셀, 가로/세로 비율과 파일 크기를 확인하고 승인 또는 거절할 수 있습니다. 최소 변은 `64px`, 전체 상한은 `40,000,000px`입니다. 승인 뒤 DWPose가 CPU에서 관절 18개를 추출하고 자세 원본·관절 겹쳐보기·표준 OpenPose 뼈대 지도 3개를 다시 사용자에게 공개합니다. 승인 지도는 비율을 유지한 채 생성 크기로 맞추며 빈 공간은 검은색으로 채웁니다. ControlNet 초기값은 강도 `0.65`, 적용 구간 `0.00~0.80`, 이미지 변경 강도 `0.35`입니다. 코드 연결은 완료했으며 실제 GUI DWPose·ControlNet 생성은 각각 `0회·0회`, 자동 저장은 `0개`입니다.
...
같은 생성 후보 원본 + 승인 마스크 + 추출 의상 → CatVTON 제한 합성
                ↓
자세 참조 원본·수치 확인 → 사용자 승인 [입력 단계 구현]
                ↓
DWPose 관절 18개 추출 → 표준 OpenPose 지도 → 사용자 승인 [코드 구현]
                ↓
ControlNet 생성 반영 [코드 연결 완료, 실제 GPU 생성 0회]
                ↓
얼굴·손 제한 영역 보정
                ↓
사용자가 전후 결과 판단
