...
- 포함: 중립화 이미지와 RGBA 투명 구멍 미리보기, 중립화 픽셀 수·비율, 원본 마스크 포함률, 마스크 안 미중립화·마스크 밖 변경 픽셀 수
...
호환 자동 경로는 SCHP 의상 마스크 닫기·5~15px 팽창을 유지합니다. 사용자 승인 SAM2 경로는 SCHP 독립 힌트가 동시에 지지하는 `MORPH_CLOSE` 추가분만 반영하며 하드 팽창을 사용하지 않습니다. 자동 보정 결과는 원본·하드 Inpaint·소프트 합성·추가·제거 마스크와 픽셀 수를 소유하고 하드·소프트 보호 및 외곽 침범은 각각 0px여야 합니다.
...
- 포함: 정확한 Human-Agnostic 이미지 1개, 원본 좌표 하드 변경 마스크와 소프트 합성 마스크, 처리 좌표 최종 model_mask, 마스크·중립화 수치
...
기존 의상 원천은 GUI의 사용자 선택 SAM2 승인 마스크이며, 호환 호출에서는 AutoMasker 후보입니다. 전체를 외곽 밖·자동 보호 접촉·보호 밖 검증 대상으로 분류합니다. 자동 보호 접촉은 제거 분모에서 제외하고 WARNING 수치와 위치로 공개하며, 보호 밖 잔여만 BLOCK합니다. 상태는 covered, incomplete, not_evaluable이며 검증 대상 0px의 removal_percent는 None입니다. 이는 의상 의미 인식이나 생성 성공을 검증하는 자료형이 아닌 선택 영역 포함 검사입니다.
...
원본 캐릭터가 입은 의상의 체형 은폐 정도를 `thin`, `normal`, `thick`, `unknown` 중 하나로 표현합니다. 문자열 입력은 명시적으로 정규화하며 목록 밖 값은 자동 추정하지 않고 거절합니다.

### BodyRestorationMaskRoles

기존 의상 제거용 이진 마스크와 두꺼운 의상에서만 생성되는 내부 체형 힌트를 분리해 소유합니다. 두께, 수축 적용 여부, 비율, 한쪽 삽입 픽셀, 제거·힌트 픽셀 수와 보호/외곽 침범 수를 포함합니다. 제거 마스크는 입력과 같은 이진 픽셀을 유지하며 내부 힌트는 반드시 제거 마스크의 부분집합입니다. `close()`는 소유한 마스크를 해제합니다.

### OriginalBodyPose

생성된 기준 캐릭터 자체에서 추출하고 승인한 신체 복원용 자세입니다. 기준 RGB 픽셀 SHA-256, 원본 크기, 독립 소유 `PoseEstimationApprovedInput`, 필수 그룹 통과 수, 유효 뼈대 픽셀 수와 DWPose 시간을 포함합니다. 외부 자세 승인 상태와 공유하지 않으며 기준 후보 해제 때 `close()`합니다. 신체 복원 직전 기준 픽셀을 재검사한 뒤 SDXL OpenPose ControlNet 입력으로 전달합니다.
...
기준 캐릭터, 승인 Human-Agnostic 시작 이미지, 승인 마스크, 흰 배경 원본 의상, IP-Adapter Plus 실제 참조 보드, 원시 출력, 보호 출력과 4배 RGB 차이맵을 메모리에 보유합니다. 신체 복원 동작에서는 실제 원본 DWPose ControlNet 입력도 별도 미리보기로 소유합니다. 마스크 하드·소프트 픽셀, Human-Agnostic 변경 픽셀, 원시·보호 결과의 기준 캐릭터 대비 마스크 안팎 변경 픽셀, 처리 시간과 사용자 승인 결과 자동 저장 0개를 기록합니다. 성공 벤치마크 스키마는 6이며 일반 의상 경로는 11~12개, `body_pose_control.png`가 추가되는 신체 복원 경로는 12~13개 파일입니다.
