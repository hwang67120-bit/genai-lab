...
def test_codex_runner_direct_route_skips_animagine_pipeline(
        tmp_path, monkeypatch):
    calls = []
    inputs = SimpleNamespace(close=lambda: calls.append("inputs_close"))
    request_image = SimpleNamespace(close=lambda: calls.append("request_close"))
    request = SimpleNamespace(reference_image=request_image)
    approval = SimpleNamespace(fingerprint="f" * 64)
    config = {
        "native_pipeline_v2": {
            "enabled": True,
    ...
    selection = SimpleNamespace(batch_directory=tmp_path)
    ...
    class FakeOrchestrator:
        ...
        def select_direct_source(self, value):
            assert value is inputs
            calls.append("select_direct")
            return selection
    ...
    monkeypatch.setattr(
        runner,
        "load_generation_replay_bundle",
        lambda path: (inputs, config, request, approval),
    )
    monkeypatch.setattr(runner, "validate_config", lambda value: None)
    monkeypatch.setattr(runner, "configure_system_certificates", lambda: None)
    monkeypatch.setattr(
        runner,
        "check_environment",
    ...
    monkeypatch.setattr(
        ...
        lambda value: (_ for _ in ()).throw(
            AssertionError("direct route loaded Animagine")
        ),
    )
    monkeypatch.setattr(
        runner, "create_generation_run_log", lambda root: FakeLog()
    )
    monkeypatch.setattr(runner, "GenerationOrchestrator", FakeOrchestrator)
    monkeypatch.setattr(runner.torch.cuda, "is_available", lambda: False)
    ...
    assert record["route"] == "source_character_direct"
    assert record["selected_engine"] == "flux2_klein"
    assert record["image_merge_used"] is False
    assert "select_direct" in calls
