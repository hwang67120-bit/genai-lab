...
- 현재 단계: 기준 후보 1장을 먼저 생성한 뒤 같은 후보에서 SCHP·DensePose·DWPose 신체 비교와 의상 변경 마스크를 만듭니다. 원시 의상 마스크는 반경 1~3px 닫기 연산 뒤 긴 변의 1.0%를 기준으로 5~15px 팽창하고 보호 영역을 뺍니다. 사용자는 `Human-Agnostic Image`와 기존 의상 잔여 마스크를 포함한 10개 화면을 확인합니다. 탐지된 기존 의상 제거율이 `100.000%`가 아니거나 잔여가 `1픽셀 이상`이면 승인을 막습니다. CatVTON에는 회색 이미지가 아니라 같은 생성 후보 원본, 승인 마스크와 추출 의상을 전달합니다. 내부 AutoMasker 실행 횟수는 0회이며 반환 마스크가 승인 마스크와 1픽셀이라도 다르면 결과를 거절합니다.
