...
- 현재 단계: 의상 전처리 5단계와 캐릭터 신체 비교 코드가 연결됐습니다. WD14 승인 뒤 SCHP·DensePose가 착의 후보와 가려진 영역의 연속 범위를 만들고, DWPose는 관절 18개의 검출 여부와 좌표를 분석합니다. 원시 의상 마스크는 반경 1~3px 닫기 연산 뒤 긴 변의 1.0%를 기준으로 5~15px 팽창하고 얼굴·손·발·피부 보호 영역을 뺍니다. 남은 영역은 RGB `(127, 127, 127)`로 중립화하며, 사용자는 기존 8개 분석 화면과 `Human-Agnostic Image`를 합친 9개 결과를 승인합니다. 승인 이미지와 정확한 변경 마스크는 메모리에 보관하지만 CatVTON이 이를 입력으로 사용하는 연결은 다음 단계이며, 현재 CatVTON 실행은 아직 내부 마스크를 다시 계산합니다.
...
현재 GUI는 의상 위치, SAM2 마스크, 원본 픽셀 추출, WD14 분석, 캐릭터 신체 비교와 Human-Agnostic 후보 확인을 순서대로 실행합니다. WD14는 점수 35.0% 이상 후보 최대 30개를 공개하고 사용자가 잘못된 태그를 제외합니다. 신체 비교는 576×1024 분석 캔버스, 관절 신뢰도 30.0%, 마스크 닫기 2px, 팽창 5~15px 값을 사용합니다. 9개 중간 결과를 승인하기 전 CatVTON 호출은 0회로 유지되며, 캐릭터 이미지 또는 의상 종류가 바뀌면 승인 이미지 1개와 변경 마스크 1개를 즉시 메모리에서 해제합니다.

기존 의상 전처리·분석 단위 테스트는 16개 통과, 0개 실패, Windows 실행 시간 12.96초였습니다. 2026-08-28 신체 비교·CatVTON 마스크 집중 검사는 15개 통과, 0개 실패, 공유 폴더 실행 시간 1.25초였습니다. 2026-08-29 Human-Agnostic 집중 검사는 9개 통과, 0개 실패, 4.89초였고 변경 파일 2개의 Python 구문 검사를 통과했습니다. 사용자는 흰 와이셔츠 1건에서 마스킹·원본 의상 추출·WD14 키워드 추출을 직접 확인했습니다. 마스킹 체감 품질 약 99%는 자동 정확도가 아니라 사용자 육안 평가 1건입니다. Human-Agnostic 실제 GUI 확인과 CatVTON 연결 시험은 아직 0회이므로 이 작은 테스트 결과를 실제 합성 품질 증명으로 기록하지 않습니다.

### 확정된 의상 추출 검토 정책

의상 추출은 다음 7개 화면을 순서대로 공개하며 중간 단계를 숨기지 않습니다.
...
과거 시험에서 일반 의상 이미지를 추출하지 않고 `전신 의상`으로 전달했을 때 몸통·다리·꼬리까지 넓게 잡힌 마스크 위에 회색 천 형태가 합성됐습니다. JPG 형식이 직접 원인은 아니었습니다. 현재는 승인된 투명 의상 추출본만 전달하고, CatVTON 실제 마스크에도 닫기 2px·팽창 5~15px·신체 보호 영역 차감을 다시 적용합니다. 그래도 DensePose와 SCHP가 애니 캐릭터를 잘못 나누면 실패할 수 있으므로 Human-Agnostic Image를 포함한 9개 미리보기 승인 절차를 제거하지 않습니다.
